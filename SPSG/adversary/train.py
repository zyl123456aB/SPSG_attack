...
def main():
    ...
    with open(transferset_samples[0][1], 'rb') as rf:
        target = pickle.load(rf)
    print(target)
    if target:
        num_classes = torch.tensor(target[0]).size(0)
    ...
    for b in budgets:
        ...
        if args.hard_label == False:
            criterion_train = model_utils.soft_cross_entropy
        else:
            criterion_train = None

        model_utils.train_model(model, blackbox, transferset, model_dir, testset=testset, criterion_train=criterion_train,
