...
import SPSG.utils.transforms as transform_utils
...
class RandomAdversary(object):
    ...
    def get_transferset(self,   budget,transfer_out_path):
        start_B = 0
        end_B = budget
        with tqdm(total=budget) as pbar:
            for t, B in enumerate(range(start_B, end_B, self.batch_size)):
                ...
                sg_t,y_t = self.blackbox.get_SG_distribution(x_t)
                ...
                for i in range(x_t.size(0)):
                    ...
                    # transferdata = ( y_t[i][0].cpu().squeeze(),y_t[i][1].cpu().squeeze(),sg_t[i].cpu().squeeze())
                    transferdata = (y_t[i].cpu().squeeze(), sg_t[i].cpu().squeeze())
                    img_t_i = img_t_i.replace("/home/zyl/SPSG_attack/data/ILSVRC2012/train/","")
                    img_t_i = img_t_i.replace(".JPEG", "")
                    img_t_i1 = img_t_i.split("/",1)[0]
                    img_t_i2 = img_t_i.split("/", 1)[1]
                    out_path = osp.join(transfer_out_path,img_t_i1)
                    logits_path  = img_t_i2+".pickle"
                    if not os.path.exists(out_path):
                        os.makedirs(out_path)
                    final_path = osp.join(out_path,logits_path)
                    print(final_path)
                    with open(final_path, "wb") as wf:
                        pickle.dump(transferdata,wf)
                    self.transferset.append((q,final_path))
                print('=> transfer set ({} samples) written to: {}'.format(self.batch_size, transfer_out_path))
...
def main():
    ...
    transferset = adversary.get_transferset(params['budget'],transfer_out_path)
    params['created_on'] = str(datetime.now())
    params_out_path = osp.join(out_path, 'params_transfer.json')
    with open(params_out_path, 'w') as jf:
        json.dump(params, jf, indent=True)
    with open(transfer_out_path+"/transferset.pickle","wb") as ww:
        pickle.dump(transferset,ww)
